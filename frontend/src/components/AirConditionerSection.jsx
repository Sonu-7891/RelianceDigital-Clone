...
const AirConditionerSection = () => {
  ...
  const loadProducts = useCallback(async () => {
    try {
      const data = await fetchProducts({ category: "AirConditioners" });
  ...
  const productSectionProps = useMemo(
    () => ({
      title: "Best in Air Conditioners",
      subtitle: "Get Up To 10% Instant Bank Discount*",
      products: products,
      bgcolor: "#c3e3f0",
    }),
    [products]
...
export default React.memo(AirConditionerSection); // Prevent re-renders if props haven't changed
