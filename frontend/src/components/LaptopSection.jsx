...
const LaptopSection = () => {
  ...
  const loadProducts = useCallback(async () => {
    try {
      const data = await fetchProducts({ category: "Laptops" });
  ...
  const productSectionProps = useMemo(
    () => ({
      ...
      subtitle: "Get Up To 15% Instant Bank Discount*",
      products: products,
      bgcolor: "#c3e3f0",
    }),
    [products]
...
export default React.memo(LaptopSection); // Prevent re-renders if props haven't changed
