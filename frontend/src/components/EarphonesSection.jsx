...
const EarphonesSection = () => {
  ...
  const loadProducts = useCallback(async () => {
    try {
      const data = await fetchProducts({ category: "Earphones" });
  ...
  const productSectionProps = useMemo(
    () => ({
      title: "Top Earphones",
      subtitle: "Upto 80% off on Best Sellers",
      products: products,
      bgcolor: "#c3e3f0",
    }),
    [products]
...
export default React.memo(EarphonesSection); // Prevent re-renders if props haven't changed
